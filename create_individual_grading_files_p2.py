...
def writeIndividualFile(template, firstName, lastName, email, total, q1, q2, q3, q4, q5, q6, q7, q8,
    ...
    table1.cell(1, 1).text = LetterGrade(total, 263)  # Grade
    table1.cell(1, 1).paragraphs[0].paragraph_format.alignment = WD_TABLE_ALIGNMENT.CENTER

    table1.cell(1, 2).text = str(int(total)) + '/263'  # Points, please remember modify the full score
    ...
    table2.cell(7, 2).text = str(int(q7))
    table2.cell(7, 2).paragraphs[0].paragraph_format.alignment = WD_TABLE_ALIGNMENT.CENTER
    table2.cell(7, 3).text = comment7

    table2.cell(8, 2).text = str(int(q8))
    table2.cell(8, 2).paragraphs[0].paragraph_format.alignment = WD_TABLE_ALIGNMENT.CENTER
    table2.cell(8, 3).text = comment8
...
if __name__ == '__main__':
    ...
    if not os.path.exists(outpath):
        os.makedirs(outpath)
    doc_template_path = 'p2_form_f22.docx'  # the template of the individual grading sheet
    excel_path = 'p2_spreadsheet_f22.xls'  # the spreadsheet file
    ...
    for i in range(start, end):  # get values of each record
        ...
        # print(comments)
        if q1 != '':
            writeIndividualFile(doc_template_path, firstName, lastName, email, total, q1, q2, q3, q4, q5, q6, q7, q8,
                                comment1, comment2, comment3, comment4, comment5, comment6, comment7, comment8)
